...
from lists.forms import (
    ItemForm, EMPTY_ITEM_ERROR, ExistingListItemForm, DUPLICATE_ITEM_ERROR, NewListForm)
...
class LiveViewTest(TestCase):
    ...
    def test_passes_correct_list_to_template(self):
        ...
        correct_list = List.objects.create()
        response = self.client.get(f'/lists/{correct_list.id}/')
        self.assertEqual(response.context['list'], correct_list)  # response.context represents the context we’re
...
class NewListViewIntegratedTest(TestCase):
    ...
    def test_can_save_a_POST_request(self):
        ...
        self.assertEqual(Item.objects.count(), 1)  # check new item has been saved to db
        new_item = Item.objects.first()  # gets first object
        self.assertEqual(new_item.text, 'A new list item')  # check item text is the same
...
class NewListViewUnitTest(unittest.TestCase):
    ...
    def test_save_form_with_owner_if_form_valid(self, mockNewListForm):
